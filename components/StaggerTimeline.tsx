...
export function StaggerTimeline({ items }: { items: readonly ExperienceItem[] }) {
  ...
  return (
    <motion.ol
      className="mt-14 space-y-0 border-l border-white/10 pl-8 md:pl-10"
      variants={container}
    ...
    >
      {items.map((item, i) => (
        <motion.li key={i} variants={row} className="relative pb-12 last:pb-0">
          <span
            className="absolute -left-[21px] top-1.5 h-2.5 w-2.5 rounded-full bg-violet-400 shadow-[0_0_12px_rgba(167,139,250,0.6)] md:-left-[25px]"
            aria-hidden
          />
          <GlassCard className="relative overflow-hidden">
            <div className="flex flex-col gap-2 gap-y-1 md:flex-row md:items-start md:justify-between">
              <p className="font-display text-lg font-semibold text-white md:text-xl">{item.title}</p>
              <NanoChip className="shrink-0 self-start md:mt-0.5">{item.period}</NanoChip>
            </div>
            <p className="mt-1 text-sm font-medium text-violet-200/80">{item.org}</p>
            <p className="mt-3 text-sm leading-relaxed text-white/55 md:text-[15px]">{item.detail}</p>
          </GlassCard>
        </motion.li>
      ))}
    </motion.ol>
