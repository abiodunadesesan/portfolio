...
export default function HeroAtmosphere({ scrollYProgress, reduceMotion }: Props) {
  ...
  const blurHero = tier === "mobile" ? "blur-[34px]" : tier === "tablet" ? "blur-[80px]" : "blur-[100px]";
  const blurMid = tier === "mobile" ? "blur-[28px]" : tier === "tablet" ? "blur-[72px]" : "blur-[90px]";
  const blurSoft = tier === "mobile" ? "blur-[24px]" : tier === "tablet" ? "blur-[64px]" : "blur-[80px]";

  const meshEnd = tier === "mobile" ? 0.08 : tier === "tablet" ? 0.118 : 0.14;
  const vignetteStops =
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-[1] overflow-hidden">
      ...
      <div
        className={`hero-noise absolute inset-0 mix-blend-overlay ${
          tier === "mobile" ? "opacity-[0.015] dark:opacity-[0.03]" : "opacity-[0.04] dark:opacity-[0.07]"
        }`}
        aria-hidden
      />
    </div>
